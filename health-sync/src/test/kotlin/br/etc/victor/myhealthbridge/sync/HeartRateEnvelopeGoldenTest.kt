 ...
 *
 * The same literal is posted at `IngestionReadModelTest` in `ingestion-api`, against a real API and a
 * real PostgreSQL, so that the two halves of the vertical are pinned to one another: what the mapper
 * emits is what the read model was proven to show. Changing it here without changing it there breaks
